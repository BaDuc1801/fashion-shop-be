...
function summarizeEvents(events) {
  ...
  for (const e of events) {
    if (e.productName)
      productCount[e.productName] = (productCount[e.productName] || 0) + 1;
    if (e.categoryName)
      categoryCount[e.categoryName] = (categoryCount[e.categoryName] || 0) + 1;
...
async function buildStyleProfile(userBehaviorModel, userId) {
  ...
  const response = await anthropic.messages.create({
    ...
    max_tokens: 250,
    system:
      "Phân tích lịch sử mua sắm quần áo, trả về preference. Chỉ trả JSON.",
    messages: [
      {
        role: "user",
